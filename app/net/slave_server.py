    ...
    def _grab_cameras(self):
        ...
        for attr, name, tint in (("profile_red", "cam_red", (1.0, 0.18, 0.18)),
            ...
            if cam is None or not hasattr(cam, "read_stripe"):
                continue
            try:
                # riktig kamera → HELA sensorramen (grab_preview, ser allt i GUI:t);
                # sim → rik tvärsnitts-preview (stripe_preview); annars mät-ROI:n
                preview = getattr(cam, "grab_preview", None) or getattr(cam, "stripe_preview", None)
